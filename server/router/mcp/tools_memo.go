...
func parseMemoUID(name string) (string, error) {
	...
	if !ok || uid == "" {
		return "", fmt.Errorf(`memo name must be in the format "memos/<uid>", got %q`, name)
	}
	return uid, nil
}
...
func parseVisibility(s string) (store.Visibility, error) {
	...
	case store.Public, store.Protected, store.Private:
		return v, nil
	default:
		return "", fmt.Errorf("visibility must be PRIVATE, PROTECTED, or PUBLIC; got %q", s)
	}
}
...
func parseRowStatus(s string) (store.RowStatus, error) {
	...
	case store.Normal, store.Archived:
		return rs, nil
	default:
		return "", fmt.Errorf("state must be NORMAL or ARCHIVED; got %q", s)
	}
}
